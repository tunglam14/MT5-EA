...
input double   BE_IN_POINT   = 160;  // Khoang gia BE
         ...
         if(profit > 0 && slPrice != openPrice)
           {
            ...
            if(type == ORDER_TYPE_BUY)
               newSL = openPrice + NormalizeDouble(BE_IN_POINT*Point(), Digits());
            if(type == ORDER_TYPE_SELL)
               newSL = openPrice - NormalizeDouble(BE_IN_POINT*Point(), Digits());

            Print("Set BE ticket: ", i, " openPrice=", openPrice, " tpPrice=", tpPrice, " slPrice=", slPrice, " newSL=", newSL);
            m_trade.PositionModify(PositionGetTicket(i), newSL, tpPrice);
           }
        }
     }
  }
